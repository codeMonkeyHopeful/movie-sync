#!/usr/bin/env python3
"""
Jellyfin Media Formatter
Formats movie files and folders to Jellyfin naming convention: Title (Year)
...
class JellyfinFormatter:
    def __init__(self):
        ...
        self.subtitle_extensions = {'.srt', '.sub', '.ass', '.ssa', '.vtt', '.idx', '.sup'}
        
        # Files to remove
        self.unwanted_extensions = {'.txt', '.url', '.lnk', '.nfo', '.jpg', '.jpeg', '.png', 
                                   '.gif', '.bmp', '.exe', '.msi', '.zip', '.rar', '.7z'}
    ...
    def extract_title_year(self, filename: str) -> Optional[Tuple[str, str]]:
        ...
        if truncated_year:
            ...
            print(f"    ⚠️  Found truncated year ({year_prefix}) - using {year_full} as default")
            return (title, year_full) if title else None
    ...
    def remove_unwanted_files(self, directory: Path) -> List[str]:
        ...
        for file_path in directory.rglob('*'):
            if file_path.is_file() and file_path.suffix.lower() in self.unwanted_extensions:
                try:
                    file_path.unlink()
    ...
    def organize_subtitles(self, movie_dir: Path, movie_name: str):
        ...
        for file_path in movie_dir.iterdir():
            if file_path.is_file() and file_path.suffix.lower() in self.subtitle_extensions:
    ...
    def process_folder_contents(self, folder_path: Path, is_last_folder: bool, folder_name: str):
...
def main():
    ...
    while True:
        user_path = input("Enter the directory path containing your media files: ").strip()
